import React from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { Link } from 'react-router-dom';
import { selectItemById } from '../../redux/cart/selectors';
import { addItem } from '../../redux/cart/slice';
import { CartItems } from '../../redux/cart/types';


type PizzaBlockProps = {
  id: string;
  name: string;
  imageUrl: string;
  price: number;
  types: number[];
  sizes: number[];
  count: number;
}

const typeNames: string[] = ['тонкое', 'традиционное'];


export const PizzaBlock: React.FC <PizzaBlockProps> = ({ id, name, price, types, sizes, imageUrl }) => {
  const [addPizzaCount, setAddPizzaCount] = React.useState<number>(0);
  const [activeType, setActiveType] = React.useState<number>(0);
  const [activeSizes, setActiveSizes] = React.useState<number>(0);

  const cartItem = useSelector(selectItemById(id));

  const dispatch = useDispatch();

  const addedCount: number = cartItem ? cartItem.count : 0;

  const onClickAdd = (): void => {
    const item: CartItems  = {
      id,
      imageUrl,
      name,
      price,
      type: typeNames[activeType],
      size: sizes[activeSizes],
      count: 0,

    };

    dispatch(addItem(item));
  };

  const clickAddButton = (): void => {
    setAddPizzaCount(addPizzaCount + 1);
    onClickAdd();
  };

  return (
    <div className="pizza-block-wrapper">
      <div className="pizza-block">
        <Link to={`/pizza/${id}`}>
          <img className="pizza-block__image" src={imageUrl} alt="Pizza" />
          <h4 className="pizza-block__title">{name}</h4>
        </Link>

        <div className="pizza-block__selector">
          <ul>
            {types.map((typeId: number) => {
              return (
                <li
                  onClick={() => setActiveType(typeId)}
                  className={activeType === typeId ? 'active' : ''}
                  key={typeId}>
                  {typeNames[typeId]}
                </li>
              );
            })}
          </ul>
          <ul>
            {sizes.map((size, index) => {
              return (
                <li
                  onClick={() => setActiveSizes(index)}
                  className={activeSizes === index ? 'active' : ''}
                  key={index}>
                  {size} см.
                </li>
              );
            })}
          </ul>
        </div>
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">от {price} грн.</div>
          <button onClick={clickAddButton} className="button button--outline button--add">
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path
                d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                fill="white"
              />
            </svg>
            <span>Добавить</span>
            {addedCount > 0 && <i>{addedCount}</i>}
          </button>
        </div>
      </div>
    </div>
  );
}


